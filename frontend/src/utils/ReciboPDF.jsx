 ...
 * (la "columna derecha": sin código de barras, con firma).
 *
 * - Solo UNA página.
 * - Cabecera con imagen (o fallback tipográfico).
 * - Nombre: "Comprobante - <NOMBRE DEL SOCIO>.pdf"
 */
export async function generarReciboPDFUnico(opts = {}) {
  ...
  // Tomamos SOLO el primer socio
  const socioInput = Array.isArray(listaSocios) && listaSocios.length > 0 ? listaSocios[0] : null;
  if (!socioInput) {
  ...
  } catch { /* si falla, usamos lo que venga de socioInput */ }
  ...
  const categoria = socioInput.nombre_categoria || categorias[socioInput.id_categoria] || '';
  const estado = socioInput.nombre_estado || estados[socioInput.id_estado] || '';
  ...
  const importeStr = (typeof socioInput.importe_total === 'number')
    ? formatARS(socioInput.importe_total)
    : (typeof socioInput.importe === 'number' ? formatARS(socioInput.importe) : (socioInput.importe || '$4000'));
...
<style>
  ...
  /* Cabecera — aún menos espacio abajo */
  ...
  /* Contenido — aún menos espacio arriba */
  ...
  /* Espacio compacto alrededor de la línea divisoria */
      ...
      <div class="recibo">
        <div class="row">
          <div class="col">
            ...
            <div class="line"><span class="bold">Grupo:</span> ${categoria} <span class="bold">Estado:</span> ${estado}</div>
          </div>
  ...
  <script>
    (function(){
      setTimeout(function(){
        ...
        html2pdf().set(opt).from(node).save().then(function(){
          window.close();
        });
      }, 250);
    })();
